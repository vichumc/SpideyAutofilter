...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton(f"🗓 {math.ceil(int(offset) / 6) + 1} / {math.ceil(total / 10)}", callback_data="pages"),
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('𝚂𝙷𝙰𝚁𝙴 𝙰𝙽𝙳 𝚂𝚄𝙿𝙿𝙾𝚁𝚃')
            else:
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('📌 Check my personal message, \n      I have sent the Movie!', show_alert=True)
        except UserIsBlocked:
            await query.answer('Unblock Spidey Bot First!😼', show_alert=True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("FIRST JOIN UPDATES CHANNEL‼️ Then Press Try Again 🔃", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"📂 Here is yor Requested File 📂 {search}"
